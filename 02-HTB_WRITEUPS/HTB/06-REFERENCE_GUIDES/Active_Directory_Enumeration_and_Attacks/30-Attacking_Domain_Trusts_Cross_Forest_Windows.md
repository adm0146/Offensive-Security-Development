...
**Why it works:** Kerberoasting only requires that you can authenticate to the KDC of the target domain. With a bidirectional forest trust, our domain credentials are accepted in the foreign domain, so we can request TGS tickets for any SPN account there and crack them offline.
...
**Why it works:** Only Domain Local Groups can contain security principals from outside the forest. If an admin from Forest A (INLANEFREIGHT) is a member of a Domain Local Group in Forest B (FREIGHTLOGISTICS), they retain those rights when authenticating across the trust. The `built-in\Administrators` group is a Domain Local Group — this is the most common way this appears.
...
**Why it works (when SID filtering is OFF):** If SID filtering is not enabled on the forest trust, a user migrated from Forest A to Forest B can have Forest A SIDs in their `sidHistory` attribute. When they authenticate across the trust, those SIDs are included in their token — giving them whatever rights those SIDs have in Forest A.

**Attacker scenario:** If you can add an admin SID from Forest A to the `sidHistory` of an account in Forest B, that account has admin rights in Forest A when it authenticates across the trust.
